...
<body>
  ...
  <div style="clear: both">
    ...
    <h1 style="float:left"><a href = "printsingle.php">Back</a></h1>
 </div>
  <div style="clear: both">
  ...
  <div class="print-scroll2">
    <table align = "left" width = "940" border = "1" style="border-collapse:seperate;empty-cells:hide"  cellpadding ="6" cellspacing="6">
  <tr>
  <?php
        $group = $_POST['group'];
        ...
        $s5 = mysql_query("select memberid,status from membergroupauction where groupid = $group and chitinstallno = $noofinstall and memberid = $member");
        while($row = mysql_fetch_assoc($s5))
        {
          if($row['status'] <> 'Paid')
          {
         $memberid = $row['memberid'];
         $s0 = mysql_query("select membername from memberinfo where memberid = $memberid");
           ...
           $s2 = mysql_query("select auctionamount,auctiondt,installamount,discount from auction where groupid = '$group' and chitinstallno = '$noofinstall'");
             while($r1 = mysql_fetch_assoc($s2))
             {
               ...
               echo "<td>
                ...
                <td style = 'padding: 205px;'><table></table</td>";
